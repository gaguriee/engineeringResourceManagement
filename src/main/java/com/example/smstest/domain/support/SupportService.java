...
import com.example.smstest.domain.support.repository.SupportTypeRepository;
...
public class SupportService {
    ...
    private Support saveSupport(Support support, List<MultipartFile> files, SupportRequest supportRequest) throws IOException, NoSuchAlgorithmException {
        ...
        if (!checkRegistrationAuthority(support)) {
    ...
    // 7일 전 이내의 지원내역인지 확인
    private boolean isRegistrableDate(Date supportDate) {
        long minRegistrationMillis = System.currentTimeMillis() - (long) (registrationPeriod + 1) * 24 * 60 * 60 * 1000;
        Date minRegistration = new Date(minRegistrationMillis);
        return supportDate.after(minRegistration);
    }

    // 등록 가능한 권한이 있는지 확인 (본인인지, 등록가능기한 이내의 지원내역인지)
    public boolean checkRegistrationAuthority(Support support) {
        ...
        // Super Admin 권한이 없고 등록가능기한 이내의 지원내역이 아닐 경우 DATE_INVALID 에러 반환
        if (!containsSuperAdmin && !isRegistrableDate(support.getSupportDate())) {
            return false;
        }
